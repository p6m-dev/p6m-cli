...
impl GithubLevel {
    ...
    pub fn from_path<PATH: AsRef<Path>>(path: PATH) -> Result<GithubLevel, Error> {
        if let Ok(org_path) = path.as_ref().strip_prefix(orgs_root()) {
            let path_elements: Vec<String> = org_path
                .components()
            ...
            let org_path = match path_elements.len() {
                ...
                _ => GithubLevel::Repository(Repository::new(
                    path_elements[0].clone(),
        ...
        Err(Error::msg(
            "You must be within your local ~/orgs/ directory.".to_owned(),
        ))
    }
    ...
    pub fn local_path(&self) -> PathBuf {
        match self {
            GithubLevel::Enterprise => orgs_root(),
            GithubLevel::Organization(organization) => org_directory(organization.name()),
            GithubLevel::Repository(repository) => {
    ...
    pub fn organization(&self) -> Option<Organization> {
        match self {
            GithubLevel::Organization(organization) => Some(organization.clone()),
            GithubLevel::Repository(repository) => Some(repository.organization()),
            _ => None,
        }
    }
    ...
    pub fn repository(&self) -> Option<Repository> {
        match self {
            GithubLevel::Repository(repository) => Some(repository.clone()),
            _ => None,
        }
    }
}
